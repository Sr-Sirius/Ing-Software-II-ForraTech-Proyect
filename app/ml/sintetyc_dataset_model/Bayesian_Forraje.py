# app/ml/sintetyc_dataset_model/Bayesian_Forraje.py
...
def generatePlot(user_ph=None, user_hum=None, user_alt=None, user_temp=None):
    ...
    Gráfica principal:
    - Scatter real
    - Curva posterior aproximada
    - Punto de predicción del usuario
    """
    ...
    ax.set_ylabel(
        "Posterior Probability P(Optimal | Terrain)",
        fontsize=12,
    )
...
def generateFeatureImportancePlot():
    """
    Importancia aproximada por separación entre clases.
    """
    ...
    if importance_scores.sum() > 0:
        ...
        importance_scores = np.zeros_like(importance_scores)
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"Mean importance ({fi.mean():.1f}%)",
    )

    ax.set_xlabel("Feature Importance (%)", fontsize=12)

    ax.set_title(
        "Naive Bayes – Feature Separation Power\n"
